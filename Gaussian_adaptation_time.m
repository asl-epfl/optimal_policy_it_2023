...
data=load('final_data/Gaussian_steady_delta=0.01_seed=1015.mat');
model=load('A_noisy_Gaussian.mat');
Epsilon=exp(log(0.01):(log(0.9)-log(0.01))/29:log(0.9));

[Tstar, PHI, T_sim, T_ASL, T_adap] = cal_adaptation_time(data, model, Epsilon);
...
function [Tstar, PHI, T_sim, T_ASL, T_adap] = cal_adaptation_time(data, model, Epsilon)
%% --------- Tstar and PHI --------
N=model.N;
A_N=model.A_N;
mu=model.mu;
...
for i=1:length(Epsilon)
    for j=1:A_N
        T_ASL(i,j)=log(K(j)/(Epsilon(i)*PHI(j,1)))/log(1/(1-delta));
    end
end
